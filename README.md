...
通过 GitHub Actions 使用 OpenWrt ImageBuilder 构建精简固件，预装 shadowsocks-libev 透明代理，支持国内外分流，适合作为翻墙网关使用。
...
- 开机自启动
- 一键开关：`/etc/init.d/shadowsocks start|stop`
...
chnroute 列表在每次 CI 构建时拉取最新版并打包进固件。需要更新时重新 push 触发构建即可。

## 构建方法

每次 push 到 main 分支会自动触发构建，等待约 3-5 分钟，在 Actions 页面下载 artifact。
...
刷入后路由器会自动重启，LAN IP 自动设为 192.168.2.1。
...
# 改 YOUR_SERVER 和 YOUR_PASSWORD
/etc/init.d/shadowsocks start
```

其他功能（分流、DNS 防污染、定时重启、进程监控）均已自动配置。

## 网络拓扑

```
互联网 ← 光猫/主路由 (192.168.1.1) ← [WAN] WR710N (192.168.2.1) [LAN/WiFi] ← 你的设备
                                              ↕
                                      shadowsocks-libev
                                    ┌─────────┴─────────┐
                                中国 IP → 直连      其他 → 代理
...
WAN 口（靠近 USB 口）接光猫/主路由 LAN 口，通过 DHCP 获取上游网络。

## 已知限制

- **不支持 DNS 广告过滤** — 设备仅 56MB 可用内存，加载大型 hosts 文件会导致内存耗尽
- **无 LuCI shadowsocks 管理界面** — Flash 空间有限，ss 通过 SSH 配置
- **加解密性能有限** — 400MHz MIPS CPU，不适合高带宽场景
...
files/
├── etc/
│   ├── init.d/shadowsocks          # init 脚本（启停 ss-redir/ss-tunnel/iptables/DNS）
│   ├── uci-defaults/99-shadowsocks # 首次启动配置（LAN IP、cron）
│   └── shadowsocks-libev/
